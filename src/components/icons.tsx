 ...
 *  at 14px, where interior detail turns to mush and only the silhouette
 *  reads. Nothing else in the set is a diagonal, so the pencil stays legible
 *  when the strokes blur together. */
export function NoteIcon({ size = 18, className }: IconProps) {
  return (
    <svg {...svgProps(size, className)}>
      <path d="M5.4 18.6l.8-3.4L16.9 4.5a2 2 0 0 1 2.9 2.9L9.1 18.1l-3.7.5z" />
      <path d="M14.7 6.7l2.9 2.9" />
    </svg>
  );
}
...
/** Tasks: a bolt — a one-shot job, fired and done. Not a checklist: the list
 *  of things to pick up later is the Scratchpad's job. One closed shape, so
 *  there is no interior detail to mush at 14px. */
export function TasksIcon({ size = 18, className }: IconProps) {
  return (
    <svg {...svgProps(size, className)}>
      <path d="M13.4 3.2 5.9 13.5h4.9l-1 7.3 7.6-10.3h-5z" />
    </svg>
  );
}
