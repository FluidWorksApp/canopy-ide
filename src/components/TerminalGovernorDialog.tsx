...
interface Props {
  ...
  /** What the host can still promise. An increment above this is refused by the
   *  governor, so offering it as a live button is offering a no-op. */
...
export function TerminalGovernorCard({
  ...
  const increments = request?.increments ?? [];
  // Mirrors the governor's own refusals (governor.rs `grant_with`), so a button
  // is offered only when clicking it would actually be honoured. Presenting a
  // live Allow that the backend then refuses leaves the card with no working
  // affirmative action and no way to tell that from a bug.
  const uncappedAllowance =
    status.base_allowance_bytes +
    status.remembered_default_bytes +
    status.granted_bytes;
  const refusalFor = (bytes: number): string | null => {
    if (bytes > headroomBytes)
      return "The host's protected reserve leaves no room for this grant.";
    if (
      status.max_allowance_bytes != null &&
      uncappedAllowance + bytes > status.max_allowance_bytes
    )
      return `Above this agent's ${fmtBytes(status.max_allowance_bytes)} maximum — raise it first.`;
    return null;
  };
  const blocked = increments.length > 0 && increments.every((b) => refusalFor(b) != null);
  ...
  return (
    ...
    >
      ...
      <div className="terminal-governor-controls">
        ...
        <div className={`terminal-governor-meta${error || blocked ? " is-error" : ""}`}>
          {error ||
            (blocked
              ? `${refusalFor(increments[0])} Raise the maximum, free memory elsewhere, or stop the terminal.`
              : `Peak ${fmtBytes(quota.peak_bytes)} · ${capability.measurement.replaceAll("_", " ")}`)}
        </div>
      </div>
      <div className="terminal-governor-actions">
        <Button onClick={onDismiss} disabled={busy}>Decide later</Button>
        {increments.map((bytes, index) => (
          <Button
            key={bytes}
            variant={index === 0 ? "accent" : "default"}
            disabled={busy || refusalFor(bytes) != null}
            title={refusalFor(bytes) ?? undefined}
