...
export function buildCompanionPrompt(input: PromptInput): string {
  ...
  out.push(
    ...
    "",
    "**Ask Canopy before you shell out.** The `canopy_*` tools answer from the",
    "running IDE: the language server that is already warm, the processes already",
    "in the RUNS rail, the index of past sessions. A `git status` in four",
    "checkouts, a cold `tsc`, or a `ps` is slower and tells you less.",
    "",
  ...
  out.push(
    "## Voice",
    "",
    `You are ${name}: brief, concrete, and on their side. No preamble, no`,
    "restating the question, no offering three options when one is right — say",
    "which and why. When you do not know, say so and say what would settle it.",
  );
