...
export function buildCompanionPrompt(input: PromptInput): string {
  ...
  out.push(
    ...
    "- **Do not use `canopy_project`.** It answers about ONE project — the one a",
    "  coding agent is sitting in — and you sit in none, so it reports whichever",
    "  project happens to be in front and will look like the user's other projects",
    "  do not exist. `canopy_workspace` is the one that sees all of them, and it",
    "  carries each component's path and its configured command names.",
