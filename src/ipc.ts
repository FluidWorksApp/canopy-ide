 ...
 *  `display: none` and has no rect to ask. PNG unless `format` asks for jpeg;
 *  `mimeType` says which came back.
 *
...
export const browserSnapshot = (
  ...
  maxWidth?: number,
  format?: "png" | "jpeg",
) => invoke<BrowserShot>("browser_snapshot", { tabId, maxWidth, format });
