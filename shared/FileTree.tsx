// Multi-root lazy file tree. Directories load on expand; fs:change events
// refresh affected directories (debounced).
//
// Shared by both shells. The desktop binds it to the Rust core over Tauri IPC
// (see src/components/FileTree.tsx); the remote portal binds the SAME component
// to its WebSocket RPC and passes `readOnly`, so a phone renders the IDE's
// actual tree — same rows, same indent guides, same chevrons, same git
// colouring, same keyboard cursor, same windowing — rather than a second tree
// that merely resembles it.
//
// Everything platform-specific arrives through `fs`. There is no import of the
...
/**
 * The filesystem this tree is looking at.
 *
 * `readDir` is the only required member. Everything else is what a *writable*
 * tree needs, and `readOnly` switches all of it off — which is exactly the
 * portal's case, and the reason a remote token is never granted
 * `fs_write_file`. An adapter that omits them cannot be asked for them.
 */
 ...
 *  shared/ must not reach into src/ — the guard test asserts the two agree. */
...
interface FileTreeProps {
  ...
  fs: FileTreeFs;
  /** Override how a directory's entries are fetched without replacing the rest
   *  of `fs`. A live-shared project passes a reader backed by the tree its
   *  owner sent over the relay. */
  readDir?: (path: string) => Promise<DirEntry[]>;
  /** File-type icon for a file name. Injected rather than imported because the
   *  desktop's set is 1250 bundled SVGs — right in a local webview, wrong to
...
function FileIcon({
  ...
  name: string;
  iconUrl?: (n: string) => string | undefined;
}) {
...
export function FileTree({
  ...
  const submitPrompt = () => {
    ...
    if (kind === "new-file") {
      ...
      void run("Create folder", () => fs.createDir!(target), dir);
    } else if (path) {
      void run("Rename", () => fs.rename!(path, target), dir);
    }
  };
  ...
  // The adapter is a prop now, so a caller that builds it inline would hand us a
  // new identity every render. Read it through a ref — the same pattern this
  // file already uses for `dirs` and `items` — so the loaders keep stable
  // identities and still call the current one. Putting `fs` in their dep arrays
  // instead would re-create every callback (and re-run the watcher effect) on
  // each render of a careless caller.
  ...
  const loadGit = useCallback(async (root: string) => {
    try {
      const status = await fsRef.current.gitStatus?.(root);
      if (!status) return;
  ...
  const loadDir = useCallback(
    async (path: string) => {
      try {
        const entries = await (readDir ?? fsRef.current.readDir)(path);
        setDirs((prev) => ({
          ...prev,
  ...
  useEffect(() => {
    ...
    const gitSub = fsRef.current.onGitChange?.(() => {
  ...
  const itemsFor = (path: string, isDir: boolean, name: string): MenuItem[] => {
    ...
    return [
      ...
      },
      {
        label: "Duplicate",
        onClick: () =>
          void run("Duplicate", () => fs.duplicate!(path), parentOf(path)),
      },
      { separator: true, label: "" },
      {
        label: "Copy Path",
      ...
      },
      {
        ...
        onClick: () =>
          void fs.reveal?.(path).catch((e) => onNotice?.(String(e))),
      },
      ...(taskMenuFor
  ...
  const emptyItems = (dir: string): MenuItem[] => [
    ...
    {
      label: "Reveal in Finder",
      onClick: () => void fs.reveal?.(dir).catch(() => {}),
    },
