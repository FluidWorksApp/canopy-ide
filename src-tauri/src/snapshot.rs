...
//! The preview is an iframe inside the main webview, so the caller passes the
//! iframe's rect in CSS pixels and the capture is cropped to it.
...
pub async fn webview_snapshot(
    ...
    capture(window, x, y, width, height, max_width.unwrap_or(1200.0)).await
}
